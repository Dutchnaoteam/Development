...
class TestSuite(wx.Frame):
    def __init__(self, *args, **kwds):
        # begin wxGlade: TestSuite.__init__
        kwds["style"] = wx.DEFAULT_FRAME_STYLE
        wx.Frame.__init__(self, *args, **kwds)
        self.colourRangeSizer_staticbox = wx.StaticBox(self, -1, "Set colour range")
        ...
        self.filterWindow.image = wx.Image('./black.png', wx.BITMAP_TYPE_PNG)
    ...
    def __set_properties(self):
        # begin wxGlade: TestSuite.__set_properties
    ...
    def __do_layout(self):
        # begin wxGlade: TestSuite.__do_layout
        ...
        colourRangeSizer = wx.StaticBoxSizer(self.colourRangeSizer_staticbox, wx.HORIZONTAL)
        ...
        colourRangeSizer.Add(grid_sizer_1, 1, wx.EXPAND, 0)
        ...
        colourRangeSizer.Add(eyedropperSizer, 1, wx.EXPAND, 0)
        mainSizer.Add(colourRangeSizer, 0, wx.EXPAND, 0)
    ...
    def test(self, event):
        ...
        # flood fill
        #testTools.floodFill(image)
        # filter image
        (bgrMin, bgrMax) = self.getRangeBgr()
        print bgrMin, bgrMax
        filter = testTools.filterImageBgr(image, bgrMin, bgrMax)
    ...
    def pickColour(self, event):
        (r, g, b) = self.paletteWindow.updatePreview()  #update selected colour
        if not self.addColourButton.IsEnabled() and not self.allZeros():
            ...
            self.rMin = r; self.rMax = r+1
            self.gMin = g; self.gMax = g+1
            self.bMin = b; self.bMax = b+1
            
        (h1,s1,v1) = self.rgbToHsv((self.rMin,self.gMin,self.bMin))
        (h2,s2,v2) = self.rgbToHsv((self.rMax,self.gMax,self.bMax))
        
        if h1 < h2 and h1+180-h2 > h2-h1:
            self.hMin.SetValue(h1); self.hMax.SetValue(h2)
        else:
            self.hMin.SetValue(h2); self.hMax.SetValue(h1)
        if s1 < s2:
            self.sMin.SetValue(s1); self.sMax.SetValue(s2)
        else:
            self.sMin.SetValue(s2); self.sMax.SetValue(s1)
        if v1 < v2:
            self.vMin.SetValue(v1); self.vMax.SetValue(v2)
        else:
            self.vMin.SetValue(v2); self.vMax.SetValue(v1)
        '''
        image = self.imageWindow.image
    ...
    def hsvSpinCtrl(self, event):
        (r1,g1,b1) = self.hsvToRgb((self.hMin.GetValue(),self.sMin.GetValue(),self.vMin.GetValue()))
        (r2,g2,b2) = self.hsvToRgb((self.hMax.GetValue(),self.sMax.GetValue(),self.vMax.GetValue()))
        if r1 < r2:
            self.rMin = r1; self.rMax = r2
        else:
            self.rMin = r2; self.rMax = r1
        if g1 < g2:
            self.gMin = g1; self.gMax = g2
        else:
            self.gMin = g2; self.gMax = g1
        if b1 < b2:
            self.bMin = b1; self.bMax = b2
        else:
            self.bMin = b2; self.bMax = b1  
    ...
    def getRangeRgb(self):
        (hsvMin, hsvMax) = self.getRangeHsv()
        (r1,g1,b1) = self.hsvToRgb(hsvMin)
        (r2,g2,b2) = self.hsvToRgb(hsvMax)
        if r1 < r2:
            rMin = r1; rMax = r2
        else:
            rMin = r2; rMax = r1
    ...
    def getRangeBgr(self):
        ((rMin,gMin,bMin),(rMax,gMax,bMax)) = self.getRangeRgb()
...
class PreviewPalette(wx.Panel):
    ...
    def OnPaint(self, evt):
        ...
        '''
        #TODO Draw gradient if not picking a colour
