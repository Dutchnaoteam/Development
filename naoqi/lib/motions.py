...
class Motions():
    ...
    def setGaitConfig( self, inputConfig ):
        ...
        maxConfig = self.motProxy.getFootGaitConfig('Max')
        minConfig = self.motProxy.getFootGaitConfig('Min')
    ...
    def setGaitConfigSimple( self, maxStepX, maxStepY, maxStepTheta, stepHeight , step_max, step_min ):
        ...
        self.motProxy.setMotionConfig([["WALK_STEP_MAX_PERIOD", step_max]]) # default 30
        self.motProxy.setMotionConfig([["WALK_STEP_MIN_PERIOD", step_min]]) # default 21
    ...
    isWalking()                  return self.motProxy.walkIsActive()    x
    ...
    def backToStand(self):
        ...
        self.motProxy.setAngles(['HeadPitch', 'HeadYaw'], [-0.4, 0.0], 0.4)
        self.motProxy.post.angleInterpolation(['LShoulderPitch', 'LShoulderRoll' ,'LElbowRoll', 'LElbowYaw'],
                                  [[-0.1, 0.8],       [1, 0.5] ,      [ -1.5],        [1.9]       ],\
                                  [[0.4, 0.8],        [0.5, 0.8],     [0.5],          [0.5]],  True)                
        self.motProxy.post.angleInterpolation(['RShoulderPitch', 'RShoulderRoll', 'RElbowRoll', 'RElbowYaw'],
                                  [[0, 0.8],          [-1.0, -0.5],   [1.5],          [-1.9]],
                                  [[0.5, 0.8],        [0.5, 0.8],     [0.5],          [0.5]],  True)
                                
        self.motProxy.setAngles(['LHipYawPitch', 'RKneePitch', 'LKneePitch', 'RHipRoll', 'LHipRoll', 'RAnkleRoll', 'LAnkleRoll'],
                    [0             ,  0           , 0          ,  0        ,  0        ,  0          ,  0          ],  0.3)
        ...
        time.sleep(0.1)
        # reset legs
        self.motProxy.setAngles(['RHipPitch', 'LHipPitch'], [ -1.5, -1.5 ],  0.3 )
        time.sleep(0.2)
        # push up with arms
        ...
        self.motProxy.setAngles(['HeadPitch', 'HeadYaw'], [0.5, 0.0], 0.4)
        t = 0.4
        names = list()
        angles = list()
        times = list()
    ...
    def cartesianRight( self, angle, x,y , interval1 = 0.1, interval2= 0.09, interval3 =0.09):
        ...
        self.motProxy.setAngles(['RAnkleRoll', 'LAnkleRoll'], [0.325, 0.325], 0.1)
        self.motProxy.setAngles(['RShoulderRoll', 'LShoulderRoll', 'LShoulderPitch', 'LElbowRoll', 'LElbowYaw'], [-0.4, 0.3 + 10 * y - angle, 0.5, -1.5, 0.3], 0.4)
        time.sleep(0.5)
        ...
        # move leg back
        self.motProxy.setPosition( 'RLeg', 1, targets[0], 0.7, almath.AXIS_MASK_ALL )
        time.sleep(0.7)
        ...
        self.motProxy.positionInterpolation( 'RLeg', 1, targets[2], almath.AXIS_MASK_ALL, [dur ], True)
    ...
    def changeHead(self, yaw,pitch):
        self.motProxy.changeAngles(['HeadYaw', 'HeadPitch'], [yaw, pitch], 0.3)
    ...
    def diveLeft(self):
        ...
        self.motProxy.angleInterpolationWithSpeed(names, angles, 1.0, True)    
        
        self.motProxy.setStiffnesses('Body', 0)
        time.sleep(2)
        
        self.motProxy.setStiffnesses('RArm', 0.8)
        self.motProxy.setStiffnesses('LLeg', 0.6)
        self.motProxy.setStiffnesses('RLeg', 0.6)
        
        # move arm around body to shift balance
        self.motProxy.angleInterpolation(['RShoulderPitch','RShoulderRoll',   'RElbowYaw',     'RElbowRoll'], 
                                  [[1.372, 2.085],  [-1.2916, -0.105], [-0.154, -0.130],[1.1888, 0.380]], 
                                  [[0.4,   0.8],    [0.4,     0.8],    [0.4,    0.8],   [0.4,    0.8]], True)

        self.motProxy.setAngles('RLeg', [-0.5,0,0,0.6,0.2,-0.2], 0.2)    
        self.motProxy.setAngles('LLeg', [-0.5,0,0,0.6,0.2, 0.2], 0.2)
        self.motProxy.setStiffnesses('RArm', 0)

        # wait until nao has taken a pose or three seconds have passed
        now = time.time()
        while self.posProxy.getActualPoseAndTime()[0] != 'Back' and \
              self.posProxy.getActualPoseAndTime()[0] != 'Belly' and \
              time.time() - now < 3.0 :
            pass
        
        self.motProxy.setStiffnesses('Body',0.8)
        if self.posProxy.getActualPoseAndTime()[0] == 'Back':
            self.backToStand()
            self.walkTo(0.1, 0.05, 1.5)

        elif self.posProxy.getActualPoseAndTime()[0] == 'Belly':
            self.bellyToStand()
            self.walkTo(-0.1, -0.05, -1.5)
        else:
    ...
    def diveRight(self):
        ...
        self.motProxy.setStiffnesses('LArm', 0.8)
        self.motProxy.setStiffnesses('LLeg', 0.6)
        self.motProxy.setStiffnesses('RLeg', 0.6)
        self.motProxy.angleInterpolation(['LShoulderPitch','LShoulderRoll', 'LElbowYaw',      'LElbowRoll'], 
                                  [[1.372, 2.085],  [1.2916, 0.105], [-0.154, -0.130], [-1.1888, -0.380]], 
                                  [[0.4,   0.8],    [0.4,    0.8],   [0.4,    0.8],    [0.4,     0.8]], True)
        self.motProxy.setAngles('LLeg', [-0.5,0,0,0.6,0.2, 0.2], 0.2)    
        self.motProxy.setAngles('RLeg', [-0.5,0,0,0.6,0.2,-0.2], 0.2)
    ...
    def footLeft(self):
        ...
        self.motProxy.angleInterpolation('Body', angles, times, True)
    ...
    def getHeadPos(self, arg = True):
        return self.motProxy.getAngles(['HeadYaw', 'HeadPitch'], arg)

    # return nao's current pose
    def getPose(self):
        return self.posProxy.getActualPoseAndTime()[0]

    # return the x-, y-, rotational speed
    def getRobotVelocity(self):
        return self.motProxy.getRobotVelocity()
    ...
    def hak(self):
        ...
        # leg over ball, heel
        self.motProxy.angleInterpolation(['LHipYawPitch', 'RHipRoll', 'RHipPitch',  'RKneePitch', 'RAnklePitch'], \
                                  [[0.0 ,0.0], [-0.5, 0],  [-1.2, -0.2], [0.2, 0.5],   [0.6, -0.4]      ], \
                                  [[0.2, 0.8],     [0.3, 0.6], [0.7, 0.8],   [0.7, 0.9],   [0.7, 0.8]   ], True)
        self.motProxy.angleInterpolation(['LHipYawPitch', 'LHipRoll', 'LHipPitch', 'LKneePitch', 'LAnklePitch', 'LAnkleRoll', 'RHipRoll', 'RHipPitch', 'RKneePitch', 'RAnklePitch', 'RAnkleRoll'], \
                                    [[0],             [0],        [-0.4],     [0.95],        [-0.55],       [0],          [0],        [-0.4],     [0.95],       [-0.55],       [0]], \
                                    [[0.65],          [0.85],     [0.85],     [0.85],         [0.85],       [0.85],       [0.85],     [0.85],      [0.85],      [0.65],        [0.75]], True)

    # MAX ANGLE 0.3!!!!
    def hakje(self, angle):
        # self.motProxy.setAngles(['RAnkleRoll', 'LAnkleRoll'], [-0.2, -0.2], 0.2)
        # time.sleep(0.2)
        # step
        self.motProxy.setStiffnesses('Body', 1)
        self.motProxy.setStiffnesses('Head', 0.5)
        ...
        self.motProxy.angleInterpolation(['LHipYawPitch', 'RHipRoll', 'RHipPitch', 'RKneePitch',
                                  ...
                                  , True)
        self.motProxy.setAngles(['LShoulderRoll', 'RShoulderRoll', 'LShoulderPitch', 'RShoulderPitch'],
                         [0.1, -0.1, 1.5, 1.5], 0.2)    
        self.motProxy.angleInterpolation(['LHipYawPitch', 'LHipRoll', 'LHipPitch', 'LKneePitch', 
    ...
    def isWalking(self):
        return self.motProxy.walkIsActive()
    ...
    def keepNormalPose(self):
        ...
        if not( self.posProxy.getActualPoseAndTime()[0] == 'Stand' 
    ...
    def kickStraight(self, angle):
        ...
        if 0 <= angle < 0.2:
            ...
            self.motProxy.angleInterpolationWithSpeed(['RShoulderPitch', 'RHipPitch', 'RKneePitch', 'RAnklePitch'], 
                                                 [1.5,               -0.7,        1.05,         -0.5],  
                                                 1.0 , True)
            
            self.motProxy.angleInterpolation(['RHipPitch', 'RKneePitch', 'RAnklePitch'], \
                                        [-0.5,         1.1,        -0.65], 
                                        [[0.25],       [0.25],     [0.25]], True)
            self.normalPose(True)
        ...
        # kick towards front, left leg    
            ...
            times =  [[0.5], [0.5], [ 0.5], [0.5], [0.5], [ 0.4,  0.8], [ 0.4, 0.8], [ 0.4, 0.8], [ 0.4], [0.5], [ 0.4], [0.4] , [0.4],   [0.4]]        
            self.motProxy.angleInterpolation(names, angles, times, True)
            self.motProxy.angleInterpolationWithSpeed(['LShoulderPitch', 'LHipPitch', 'LKneePitch', 'LAnklePitch'],
                                                 [1.5,               -0.7,        1.05,         -0.5],        
                                                 1.0, True)
            self.motProxy.angleInterpolation(['LHipPitch', 'LKneePitch', 'LAnklePitch'], 
                                        [-0.5,         1.1,          -0.65], 
                                        [[0.25],      [0.25],        [0.25]], True)    
            self.normalPose(True)
    ...
    def killKnees(self):
        if self.motProxy.getStiffnesses('RKneePitch')[0] > 0:
            self.motProxy.post.stiffnessInterpolation(['RKneePitch', 'LKneePitch'], 
                                               [[0], [0]], [[0.15], [0.15]])
            self.motProxy.angleInterpolation(['RHipPitch', 'LHipPitch'], 
    ...
    def lKickAngled(self, angle):
        ...
        self.motProxy.post.angleInterpolation(['LHipYawPitch', 'RHipRoll', 'RKneePitch', 
        ...
        time.sleep(0.5 + 0.3 * angle)
        # return to start position
        self.motProxy.post.angleInterpolation(['LHipYawPitch', 'RHipRoll', 'RHipPitch', 'RKneePitch', 
                                   'LHipRoll', 'LHipPitch', 
                                   'LKneePitch', 'LAnklePitch'], 
                                   [[0],   [0],   [-0.4], [0.95], [0],   [-0.4], [0.95], [-0.55]], 
                                   [[0.65],[0.85],[0.85], [0.85], [0.85],[0.85], [0.85],[0.65]], 
                                   True)
        time.sleep(1)
    ...
    def postWalkTo(self, x, y, angle):
        self.motProxy.post.walkTo(x, y, angle,self.gaitConfig)
        
    # right kick with inputangle      
    def rKickAngled(self, angle):
        self.motProxy.setAngles(['LShoulderRoll', 'RShoulderRoll'], 
                                 [0.35,           -0.25,         ], 0.3)
        time.sleep(0.5)
        # Left leg stretched, right leg goes backwards
        self.motProxy.post.angleInterpolation(
            ['LHipYawPitch', 'LHipRoll',  'RHipRoll',          'LHipPitch',         'LKneePitch', 'LAnklePitch', 'RHipPitch', 'RKneePitch',        'RAnklePitch'], 
            [[0.65*angle],   [0.2*angle], [0.075-0.295*angle], [-0.3 - 0.05*angle], [0.7],        [-0.3],        [0.4],       [0.5 - 0.35*angle],  [-0.6 + 0.5*angle]],
            [[1.0],          [1.1],       [0.9],               [1.4],               [1.4],         [1.4],        [0.9],       [1.4],               [1.5]             ], 
            True)
        time.sleep(2.25)
        # kick
        ...
        self.motProxy.setAngles(['LHipYawPitch', 'LHipRoll', 'RHipPitch',     'RKneePitch', 'RAnklePitch'],
                                [0.75*angle,      0.1,       -0.1-0.25*angle , 0,            0.1 - 0.075*angle], 1)
        time.sleep(2 + 0.3 * angle)
        # return to start position
        self.motProxy.post.angleInterpolation(
            ['LHipYawPitch', 'LHipRoll', 'LHipPitch', 'LKneePitch', 'RHipRoll', 'RHipPitch', 'RKneePitch', 'RAnklePitch', 'RAnkleRoll'],
            [[0],            [0],        [-0.4],      [0.8],        [0],        [-0.4],       [0.95],      [-0.55],       [0] ],
            [[0.65],         [0.85],     [0.85],      [0.85],       [0.85],     [0.85],       [0.85],      [0.65],        [0.6]  ], True)
        time.sleep(0.85)
        
    # non blocking call, absolute
    def setHead(self, yaw, pitch):
        self.motProxy.setAngles(['HeadYaw', 'HeadPitch'], [yaw, pitch], 0.7)    
    ...
    def sideLeftKick(self):
        ...
        angles = [[-1],         [0.0, 0.45], [-0.4, -0.8],[0.95, 0.2], [-0.55, 0.6], [0.05 ],   [-0.4],     [0.95],     ]
        times  = [[0.2],          [0.3, 0.75],[0.5,   1.0],[0.5,  1.0], [ 0.5,  1.0] ,[ 1.0],    [0.5],      [0.5 ],    ]
        
        self.motProxy.post.angleInterpolation(names, angles, times, True)
        time.sleep(1.3)
        
        self.motProxy.post.angleInterpolationWithSpeed('LHipRoll', [-0.1], 1.0, True)
        time.sleep(1.0)
    ...
    def sideRightKick(self):
        ...
        angles = [[1],            [0,  -0.45],[-0.4, -0.8],[0.95, 0.2],[-0.55, 0.6],  [-0.05],   [-0.4],     [0.95]     ]
        times =  [[0.2],          [0.3, 0.75 ],[0.5, 1], [0.5, 1],     [0.5, 1],       [1.0],     [0.5],      [0.5]     ]
        
        self.motProxy.post.angleInterpolation(names, angles, times, True)
        time.sleep(1.3)
        self.motProxy.post.angleInterpolationWithSpeed('RHipRoll', [0.1], 1.0, True)
        time.sleep(1.0)
        self.motProxy.post.angleInterpolation(['RHipRoll','RHipPitch', 'RKneePitch', 'RAnklePitch'], 
                                    [[0.0],     [-0.6],     [1.3],        [-0.6]], 
                                    [[0.5],     [0.6],       [0.6],        [0.6]], True)
        time.sleep(0.6)
    ...
    # keeper stance
    def stance(self):
        anglelist = self.motProxy.getAngles(['RHipPitch','RKneePitch', 'RAnklePitch', 'LHipPitch', 'LKneePitch'],True)
    ...
    # activate stiffness
    def stiff(self):
        if self.motProxy.getStiffnesses('HeadPitch') != 0:
            self.motProxy.setStiffnesses(['RArm', 'LArm', 'LLeg', 'RLeg', 'Head'],
    ...
    def stiffKnees(self):
        if self.motProxy.getStiffnesses('RKneePitch')[0] < 0.95:
            self.motProxy.post.stiffnessInterpolation(['RKneePitch', 'LKneePitch'], 
                                               [[0.95], [0.95]], [[0.15], [0.15]])
            self.motProxy.angleInterpolation(['RHipPitch', 'LHipPitch'], 
        ...
        self.motProxy.setStiffnesses(['RKneePitch','LKneePitch','RAnklePitch','LAnklePitch'], 0.95)
        while self.motProxy.getStiffnesses ('RKneePitch') < 0.95:
            pass
    ...
    def setFootSteps( self, legName, footSteps, timeList, clearExisting = True ):
        ...
        #timeList = [0.6, 1.2]
        #clearExisting = False
        self.motProxy.setFootSteps(legName, footSteps, timeList, clearExisting)
    ...
    def setWalkTargetVelocity(self,x,y,t,f):
        ...
        y = float( y )
        t = float( t )
        self.motProxy.setWalkTargetVelocity(x,y,t,f,self.gaitConfig)
