...
class ParticleFilter():
    ...
    def createSamples( self, position ):
        ...
        for i in range(self.num_samples):
            if type(position) != list:
                ...
                theta = minimizedAngle( position[2] + gauss(0, 1.5) )
            self.samples.append( [ x, y, theta ] )
            self.weights.append( random() )
        self.meanState = self.findBest()
        ...
        if length == 0:
            ...
            for i in range(len(measurements)):
                ...
                for idFeature in self.worldMap:
                    ...
                    bearingR = minimizedAngle( atan2( yR-y, xR-x)+ theta )

                    sigmaRange = 0.3          # range is very uncertain
                    sigmaBearing =  0.005     # bearing is very precise

                    curChance = prob( rangeR-rangeM , sigmaRange ) * \
                                prob( bearingR-bearingM , sigmaBearing )
    ...
    def iterate( self, measurement, control):
        ...
        if measurement:
            ...
            # RESAMPLING
            wfactor = self.wfast/ self.wslow
            for m in xrange(self.num_samples):
                # If wfast/wslow is large, pick random samples to tackle the
                # kidnapping problem
                if random() < max(0, 1 - wfactor):
    ...
    def motionModel( self, sample, control ):
        ...
        theta = minimizedAngle(sample[2] + control[2])
        return x, y, theta
    ...
    def sensorModel( self, state, measurement ):
        ...
        if -0.7 < x < 6.7 and -0.7 < y < 4.7:
            ...
            weight = prob(rangeR-rangeM, sigmaRange) * \
                     prob(bearingR-bearingM, sigmaBearing)
        else:
            weight = 0
    ...
    def findBest( self ):
        ...
        bestSoFar = self.samples[0]
        bestWeight = self.weights[0]
        for s, w in itertools.izip( self.samples, self.weights ):
            if w > bestWeight:
                bestWeight = w
                bestSoFar = s
        return bestSoFar
